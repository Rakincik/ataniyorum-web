...
export default function EditInstructorForm({ instructor }: { instructor: any }) {
  const router = useRouter();
  const { showToast } = useToast();
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [imageUrl, setImageUrl] = useState(instructor.image || "");
  const [selectedColor, setSelectedColor] = useState(instructor.color || "bg-blue-500");
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showToast("Eğitmen bilgileri güncellendi.", "success");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Eğitmeni Düzenle</h1>
          <p className="text-sm text-gray-500 mt-1">{instructor.name} bilgilerini güncelleyin.</p>
        </div>
      </div>
